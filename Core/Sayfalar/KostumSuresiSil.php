...
$SilinecekSureIDsi                  = $_POST["SilinecekKostumSuresiIDsi"];
...
if((isset($ServerID)!="") && (isset($KullaniciAdi)!="") && (isset($EmailAdresi)!="") && (isset($Sifre)!="") && (isset($SilinecekSureIDsi)!="")){
    ...
    if ($UyeKontrolSonuc > 0) {
        if($UyeKontrolKayitlari["web_admin"] == 1){
            $KostumSuresiniSil = $globalDB->db->prepare("DELETE FROM global.serverkostumsuresi WHERE ServerID = ? AND id = ? LIMIT 1");
            $KostumSuresiniSil->execute([$ServerID,$SilinecekSureIDsi]);
            $KostumSuresiniSilSayi = $KostumSuresiniSil->rowCount();
            if($KostumSuresiniSilSayi>0){
                echo json_encode("BELIRTILEN_KOSTUM_SURESI_SILINDI");
            }else{
                echo json_encode("BELIRTILEN_KOSTUM_SURESI_SILINEMEDI");
            }
        }else{
