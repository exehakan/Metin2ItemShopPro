    ...
    if ($IteminToplamAdedi > 0 && $IteminToplamAdedi != "" && $IteminToplamAdedi != 0) {
        ...
        foreach ($NesneMarketSlotKontrolleriKayitlari as $key => $item) {
                ...
                for ($i = 0; $i < count($PosIndexSiralamasi); $i++) {
                    if (!in_array($PosIndexSiralamasi[$i], $PosSonuclari)) {
                        ...
                        if ($ItemAdediniBirDusurSayi > 0) {
                            ...
                            $ItemGonderilemediBirArttir = $globalDB->db->prepare("UPDATE global.itemdepo SET itemSayisi = itemSayisi + ? WHERE account_id = ? AND itemVnum = ? AND itemSayisi = ? AND id = ? AND ServerID = ? ");
                            $ItemGonderilemediBirArttir->execute([1, $AccountID, $Vnum, $Adet, $ItemID, $ServerID]);
                            $ItemGonderilemediBirArttirSayi = $ItemGonderilemediBirArttir->rowCount();
                            if($ItemGonderilemediBirArttirSayi>0){
                                echo json_encode("ITEM_GERI_IADE_EDILDI");
                            }else{
                                 echo json_encode("ITEM_GERI_IADE_EDILEMEDI_VE_ITEM_GONDERILEMEDI");
                            }
                            }
                        } else {
            ...
            if ($ItemAdediniBirDusurSayi > 0) {
                ...
                $ItemGonderilemediBirArttir = $globalDB->db->prepare("UPDATE global.itemdepo SET itemSayisi = itemSayisi + 1 WHERE account_id = ?  AND ServerID = ? AND id = ?");
                    $ItemGonderilemediBirArttir->execute([$AccountID, $ServerID,$ItemID]);
                    $ItemGonderilemediBirArttirSayi = $ItemGonderilemediBirArttir->rowCount();
                    if($ItemGonderilemediBirArttirSayi>0){
                        echo json_encode("ITEM_GERI_IADE_EDILDI");
                    }else{
